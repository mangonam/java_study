...
/*
    ...
    Arrays 클래스를 사용하고, 정적 메서드 .toString(배열명)을 사용하면 배열 전체를 출력할 수
    있습니다.
    클래스명.메서드명()이라는 점에 주목하셔야합니다 -> scanner.next()와 좀 차이가 있는 상황
    toString()이라는 메서드도 추후에 사용할 메서드입니다.

    여기서 기억해야 할 점은
    동일한 method 명이라고 하더라도 어던 클래스에 종속돼있는가에 따라서 다른 결과값을 가질 수
    있다는 점입니다.
    ex)
...
//        System.out.println(num2);
        ...
        // 이상의 코드는 눈속임이 불가능합니다. 굳이 쓰려면 가능하겠지만 맨마지막 인덱스는
        // 따로 출력해야겠지요.
